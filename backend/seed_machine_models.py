"""
Seed machine models from machineModels.js into MongoDB
"""
import asyncio
from motor.motor_asyncio import AsyncIOMotorClient
import os
from dotenv import load_dotenv
from pathlib import Path
import uuid

ROOT_DIR = Path(__file__).parent
load_dotenv(ROOT_DIR / '.env')

mongo_url = os.environ['MONGO_URL']
db_name = os.environ['DB_NAME']

client = AsyncIOMotorClient(mongo_url)
db = client[db_name]

# Complete machine models from unitedskidtracks.com
TRACK_LOADER_MODELS = {
    'ASV': ['2810', '4810', 'HD4520', 'MD70', 'MD70 Turbo', 'PT50', 'PT60', 'PT70', 'PT80', 'PT100', 'PT100 Forestry', 'RC30', 'RC50', 'RC60', 'RC85', 'RC100', 'RT25', 'RT30', 'RT40', 'RT50', 'RT60', 'RT65', 'RT65 MAX', 'RT75', 'RT75 MAX', 'RT75HD MAX', 'RT120', 'RT120 Forestry', 'RT135', 'RT135 Forestry', 'SR70', 'SR80', 'VT70', 'VT70 HO', 'VT70 HO MAX'],
    'Barreto': ['1324TK', '1624TK', '1824TK', '1836TK', '2024TK', '20RTK', '2324TK', '23RTK', '23TKD', '825TKL'],
    'Baumalight': ['TRL620D', 'TRL620Y'],
    'Bobcat': ['864', 'MT52', 'MT55', 'MT85', 'MT100', 'T62', 'T64', 'T66', 'T76', 'T86', 'T110', 'T140', 'T180', 'T190', 'T200', 'T250', 'T300', 'T320', 'T450', 'T550', 'T590', 'T595', 'T630', 'T650', 'T740', 'T750', 'T770', 'T870'],
    'Boxer': ['118', '320', '322', '322D', '375D', '385D', '400', '420', '600HD', '700HDX', 'TD327'],
    'Case': ['420CT', '440CT', '445CT', '450CT', 'TL1000', 'TR270', 'TR270B', 'TR310', 'TR310B', 'TR320', 'TR340', 'TR340B', 'TV370', 'TV370B', 'TV380', 'TV450', 'TV450B', 'TV620B'],
    'Caterpillar': ['239D', '239D3', '247', '247B', '247B2', '247B3', '249B', '249D', '249D3', '255', '257', '257B', '257B2', '257B3', '257D', '259B', '259B3', '259D', '259D3', '265', '267', '267B', '267B2', '277', '277B', '277C', '277D', '277E', '279C', '279D', '279D2', '287', '287B', '287C', '287D', '289C', '289C2', '289D', '289D2', '289D3', '297C', '297D', '297D2', '299C', '299D', '299D2', '299D3', '299D2XHP', '299D3XE'],
    'Dingo': ['220', '222', '223', '323', '325', '427', '427XL', '525', '525XL', '525R', '527R', 'K92', 'K93', 'K94', 'K93-4', 'K94-4'],
    'Ditch Witch': ['SK650', 'SK750', 'SK755', 'SK800', 'SK850', 'SK1050', 'SK3000'],
    'GEHL': ['RT105', 'RT125', 'RT165', 'RT175', 'RT210', 'RT250', 'RT255', 'RT257', 'RT260', 'RT265', 'RT270', 'V180', 'V270', 'V330'],
    'Hitachi': ['CG35', 'CG40', 'CG45', 'CG50', 'CG55', 'CG60', 'CG65'],
    'Hyundai': ['HSL650-7', 'HSL810-7', 'HSL850-7A', 'HSL1000T-7A', 'HSL1200T-7A'],
    'IHI': ['CL20', 'CL25', 'CL27', 'CL30', 'CL35', 'CL40', 'CL45', 'CL50'],
    'JCB': ['150T', '180T', '190T', '205T', '220T', '260T', '300T', '320T', '325T', '330T', '370'],
    'John Deere': ['315D', '317', '317G', '319D', '319E', '323D', '323E', '325G', '329D', '329E', '331D', '331E', '331G', '333D', '333E', '333G', 'CT322', 'CT332'],
    'Kobelco': ['SK25SR', 'SK27SR', 'SK30SR', 'SK35SR', 'SK45SR', 'SK55SR'],
    'Komatsu': ['CK20', 'CK25', 'CK30', 'CK35'],
    'Kubota': ['SCL1000', 'SCL1000R', 'SVL65', 'SVL65-2', 'SVL75', 'SVL75-2', 'SVL75-2C', 'SVL75-3', 'SVL90', 'SVL90-2', 'SVL95', 'SVL95-2', 'SVL95-2S', 'SVL95-3', 'SVL97', 'SVL97-2', 'SVL97-2C', 'SVL97-3'],
    'Mustang': ['MTL16', 'MTL20', 'MTL25', 'MTL316', 'MTL320', 'MTL325', '1500R', '1700RT', '1750R', '1750RT', '2050RT', '2100RT', '2700RT'],
    'New Holland': ['C175', 'C185', 'C190', 'C227', 'C232', 'C238', 'C245', 'C262', 'C280', 'C296', 'C314', 'C334', 'C337', 'C354'],
    'Takeuchi': ['TL6', 'TL6R', 'TL8', 'TL8R2', 'TL10', 'TL10V2', 'TL12', 'TL12R2', 'TL12V2'],
    'Terex': ['PT30', 'PT50', 'PT60', 'PT70', 'PT80', 'PT100', 'PT110', 'R160', 'R165', 'R190', 'R350'],
    'Terramac': ['RT6', 'RT9', 'RT14', 'RT14R'],
    'Toro': ['TX427', 'TX525', 'Dingo 220', 'Dingo 323', 'Dingo 427', 'Dingo 525'],
    'Vermeer': ['S450TX', 'S650TX', 'S750TX', 'S800TX', 'S925TX'],
    'Volvo': ['MCT85C', 'MCT110C', 'MCT125C', 'MCT135C', 'MCT145C'],
    'Wacker Neuson': ['ST20', 'ST25', 'ST28', 'ST31', 'ST35', '901S', '1501S', '1701S', '1801S', '2001S', '2503', '2703', '3503', '4503', '5503'],
    'Yanmar': ['T174', 'T210', 'T320', 'T475'],
}

MINI_EXCAVATOR_MODELS = {
    'Bobcat': ['320', '322G', '323', '324', '325', '325G', '331', '331D', '331E', '331G', '334', '334D', '334G', '335', '337', '337G', '341', '418', '430', '442', 'E10', 'E10E', 'E10Z', 'E17', 'E19', 'E20', 'E20Z', 'E26', 'E26R', 'E32', 'E32i', 'E35', 'E35i', 'E35R2', 'E42', 'E42R2', 'E45', 'E50', 'E50R2', 'E55', 'E60', 'E60R2', 'E63', 'E80', 'E85', 'X331'],
    'CASE': ['CX14', 'CX17B', 'CX17C', 'CX25', 'CX27B', 'CX30C', 'CX31B', 'CX36B', 'CX50', 'CX55B', 'CX57C', 'CX60C', 'CX75CSR', 'CX80C'],
    'Caterpillar': ['301.4C', '301.5', '301.6', '301.7CR', '301.7D', '301.8', '301.8C', '302.5', '302.5CR', '302.7D', '302.7DCR', '303.5CCR', '303.5DCR', '303.5E2CR', '303.5ECR', '303CCR', '303CR', '303ECR', '304CCR', '304CR', '304DCR', '304E2CR', '304ECR', '305', '305.5DCR', '305.5E', '305.5E2', '305.5E2CR', '305.5ECR', '305CCR', '305D CR', '305E2', '305E2CR', '305ECR', '306', '308E2CR'],
    'Doosan': ['DX19', 'DX27', 'DX27Z', 'DX30', 'DX30Z', 'DX35', 'DX35Z', 'DX55', 'DX60', 'DX60R', 'DX62R', 'DX63R', 'DX80', 'DX80R', 'DX85R'],
    'Gehl': ['503Z', 'Z35'],
    'John Deere': ['17D', '17G', '26G', '27CZTS', '27D', '30G', '35C', '35D', '35G', '35ZTS', '50D', '50G', '60D', '60G'],
    'Kubota': ['KX018-4', 'KX033', 'KX033-4', 'KX040', 'KX040-4', 'KX057-4', 'KX057-5', 'KX080-3', 'KX080-4', 'KX080-5', 'KX121-2', 'KX121-3', 'KX161-3', 'KX21', 'KX41-2', 'KX41-2V', 'KX61-2', 'KX71-3', 'KX91', 'KX91-2', 'KX91-3', 'U15', 'U17', 'U25-3', 'U35-4', 'U35S', 'U45', 'U45-3', 'U55', 'U55-4'],
    'New Holland': ['EC15', 'EC35'],
    'SANY': ['SY35U', 'SY50', 'SY60C', 'SY75C', 'SY80U'],
    'Takeuchi': ['TB008', 'TB015', 'TB016', 'TB025', 'TB035', 'TB080', 'TB125', 'TB135', 'TB138FR', 'TB145', 'TB153FR', 'TB175', 'TB180FR', 'TB230', 'TB235', 'TB240', 'TB250A', 'TB260', 'TB280FR', 'TB285', 'TB370', 'TB53FR'],
    'Volvo': ['EC15B', 'EC20', 'EC25', 'EC35', 'EC45', 'EC55', 'EC55B', 'EC55C', 'ECR25D', 'ECR38', 'ECR40D', 'ECR48C', 'ECR58', 'ECR88'],
    'Wacker Neuson': ['28Z3', '3503', '38Z3']
}


async def seed_machine_models():
    """Seed all machine models into database"""
    
    # Clear existing machine models
    await db.machine_models.delete_many({})
    print("🗑️  Cleared existing machine models")
    
    total_added = 0
    
    # Seed Track Loader models
    print("\n📦 Seeding Track Loader Models...")
    for brand, models in TRACK_LOADER_MODELS.items():
        for model in models:
            doc = {
                "id": str(uuid.uuid4()),
                "brand": brand,
                "model": model,
                "equipment_type": "Track Loader",
                "created_at": "2024-01-01T00:00:00Z",
                "updated_at": "2024-01-01T00:00:00Z"
            }
            await db.machine_models.insert_one(doc)
            total_added += 1
        print(f"  ✅ {brand}: {len(models)} models")
    
    # Seed Mini Excavator models
    print("\n📦 Seeding Mini Excavator Models...")
    for brand, models in MINI_EXCAVATOR_MODELS.items():
        for model in models:
            doc = {
                "id": str(uuid.uuid4()),
                "brand": brand,
                "model": model,
                "equipment_type": "Mini Excavator",
                "created_at": "2024-01-01T00:00:00Z",
                "updated_at": "2024-01-01T00:00:00Z"
            }
            await db.machine_models.insert_one(doc)
            total_added += 1
        print(f"  ✅ {brand}: {len(models)} models")
    
    # Summary
    print(f"\n✅ Total machine models seeded: {total_added}")
    
    # Count by equipment type
    track_loader_count = await db.machine_models.count_documents({"equipment_type": "Track Loader"})
    mini_ex_count = await db.machine_models.count_documents({"equipment_type": "Mini Excavator"})
    
    print(f"   📊 Track Loaders: {track_loader_count}")
    print(f"   📊 Mini Excavators: {mini_ex_count}")
    
    # Get brand counts
    brands = await db.machine_models.distinct("brand")
    print(f"   📊 Total Brands: {len(brands)}")


async def main():
    """Main function"""
    print("\n🚀 Seeding Machine Models from unitedskidtracks.com...\n")
    
    try:
        await seed_machine_models()
        print("\n✅ Machine models seeding complete!\n")
    except Exception as e:
        print(f"\n❌ Error during seeding: {str(e)}")
    finally:
        client.close()


if __name__ == "__main__":
    asyncio.run(main())
